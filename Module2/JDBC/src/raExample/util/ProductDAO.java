...
import static raExample.util.ConnectionDB.getConnection; // Nhập phương thức getConnection từ lớp ConnectionDB để lấy kết nối CSDL.

public class ProductDAO { // Khai báo lớp ProductDAO để quản lý các thao tác liên quan đến sản phẩm trong CSDL.
    ...
    public void updateProduct(int id, String productName, double price, String description) { // Phương thức cập nhật thông tin sản phẩm dựa trên ID.
        String sql = "UPDATE Product SET product_name = ?, price = ?, description = ? WHERE id = ?"; // Câu lệnh SQL để cập nhật sản phẩm.
        try (Connection conn = getConnection(); // Lấy kết nối tới CSDL và tự động đóng sau khi hoàn thành.
    ...
    // Lấy tất cả sản phẩm
    public void getAllProducts() { // Phương thức lấy danh sách tất cả sản phẩm.
        String sql = "SELECT * FROM Product"; // Câu lệnh SQL để chọn tất cả sản phẩm.
        try (Connection conn = getConnection(); // Lấy kết nối tới CSDL và tự động đóng sau khi hoàn thành.
            ...
            while (rs.next()) { // Lặp qua các bản ghi trong ResultSet.
                int id = rs.getInt("id"); // Lấy giá trị id từ bản ghi hiện tại.
                String productName = rs.getString("product_name"); // Lấy giá trị product_name từ bản ghi hiện tại.
                double price = rs.getDouble("price"); // Lấy giá trị price từ bản ghi hiện tại.
                String description = rs.getString("description"); // Lấy giá trị description từ bản ghi hiện tại.
                // In ra thông tin sản phẩm.
                System.out.println("ID: " + id + ", Name: " + productName + ", Price: " + price + ", Description: " + description);
            }
        } catch (SQLException e) { // Xử lý ngoại lệ có thể xảy ra.
            e.printStackTrace(); // In ra chi tiết lỗi nếu có ngoại lệ.
        }
